...
'''
Start here, IdVtg with different back gate


'''
def read_csv_IdVtg_diffBG(filename):
    df  = pd.read_csv(filename, skiprows=256)
    df.drop(df.columns[0], axis=1, inplace=True)
...
def reshape_table_by_VG_diffBG(df):
    df_new = pd.pivot_table(df,columns=' Vbg',values=[' ID'],index=[' VTG',' forward'])
    df_sorted = df_new.sort_values(by=[' forward',' VTG'])
    return df_sorted


def reorder_by_VG_diffBG(df_sorted, Vbg):
    midpoint = len(df_sorted) // 2
...
IdVtg with double gate 


'''

def read_csv_IdVtg_double(filename):
    df  = pd.read_csv(filename, skiprows=256)
    df.drop(df.columns[0], axis=1, inplace=True)
    Vbg = df.iloc[1, 2]
    df.drop(columns=[' Vbg', ' IS'], inplace=True)
...
def reshape_table_by_VD_double(df):
